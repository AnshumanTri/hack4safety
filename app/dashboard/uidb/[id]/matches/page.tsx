...
import { mockUIDBs, mockMatches, mockMissingPersons } from "@/lib/mock-data"
import { ArrowLeft, AlertCircle, CheckCircle, XCircle, Eye } from "lucide-react"
import Link from "next/link"

export default function UIDBMatchesPage() {
  const params = useParams()
  const uidb = mockUIDBs.find((u) => u.id === params.id) || mockUIDBs[0]
  const matches = mockMatches.filter((m) => m.uidbId === uidb.id)
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight">AI Matches for {uidb.caseNumber}</h1>
          <p className="text-muted-foreground mt-1">Potential missing person matches based on AI analysis</p>
        </div>
      </div>
      ...
      {matches.length === 0 ? (
        <Card className="border-2">
          <CardContent className="py-12 text-center">
            ...
            <p className="text-muted-foreground mt-2">
              The AI system is continuously analyzing missing person records for potential matches
            </p>
          </CardContent>
        </Card>
      ) : (
        <div className="space-y-6">
          {matches.map((match) => {
            const person = mockMissingPersons.find((p) => p.id === match.missingPersonId)
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <div className="flex gap-4">
                        <div className="w-32 h-32 rounded-lg overflow-hidden border-2 relative">
                          <img
